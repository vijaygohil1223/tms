...
class STM_LMS_Compatibility {
	...
	private static $pro   = '3.7.8';

	public function __construct() {
		add_action( 'admin_notices', array( $this, 'init' ) );
	}

	public function check_theme_version( $value ) {
		$theme = wp_get_theme();
		if ( 'MasterStudy' == $theme->name ) {
			if ( 'pro' === $value ) {
				return version_compare( '4.5.1', $theme->version ) > 0;
			} elseif ( 'theme' === $value ) {
	...
	public function init() {
		...
		if ( $this->check_theme_version( 'theme' ) ) { ?>
		...
		if ( $this->check_pro_plugin_version() ) {
			...
			<div class="notice notice-lms notice-lms-go-to-pages notice-lms-compatibility">
				...
				<?php if ( stm_lms_is_masterstudy_theme() && $this->check_theme_version( 'pro' ) ) { ?>
					<div class="notice-lms-go-to-pages-content">
						<p>
							<strong>
				...
				<?php } elseif ( stm_lms_is_masterstudy_theme() && ! $this->check_theme_version( 'pro' ) ) { ?>
					<div class="notice-lms-go-to-pages-content">
						<p>
							<strong>
