...
class STM_LMS_User {
	...
	public static function verify_user() {
		...
		wp_redirect( self::login_page_url() );
	}
	...
	public static function _get_user_courses( $offset, $sort = 'date_low' ) {
		...
		if ( ! empty( $courses ) ) {
			foreach ( $courses as $course ) {
				...
				$membership_status   = STM_LMS_Subscriptions::get_membership_status( get_current_user_id() );
				$not_salebale        = get_post_meta( $id, 'not_single_sale', true );
				$membership_expired  = ( 'expired' == $membership_status && $not_salebale );
				$membership_inactive = ( 'active' !== $membership_status && 'expired' !== $membership_status && $not_salebale );
	...
	public static function has_course_access( $course_id, $item_id = '', $add = true ) {
		...
		/*Check for membership expiration*/
		$membership_status   = STM_LMS_Subscriptions::get_membership_status( $user_id );
		$membership_expired  = ( 'expired' == $membership_status && $not_salebale );
		$membership_inactive = ( 'active' !== $membership_status && 'expired' !== $membership_status && $not_salebale );
		...
		if ( ! count( $course ) ) {
			...
			if ( empty( $course_price ) && ! $not_salebale && $prerequisite_passed && $add ) {
				STM_LMS_Course::add_user_course( $course_id, $user_id, STM_LMS_Course::item_url( $course_id, '' ), 0 );
