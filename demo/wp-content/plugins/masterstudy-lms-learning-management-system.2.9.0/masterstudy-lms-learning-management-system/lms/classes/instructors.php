...
class STM_LMS_Instructor extends STM_LMS_User {
	...
	public static function init() {
		...
		add_filter(
			'stm_lms_float_menu_items',
			function ( $menus, $current_user, $lms_template_current, $object_id ) {
				...
				if ( self::instructor_can_add_students() && self::is_instructor() ) {
					$menus[] = array(
						...
						'label'                => esc_html__( 'Add student', 'masterstudy-lms-learning-management-system' ),
	...
	public static function change_status() {
		...
		if ( empty( $_GET['post_id'] ) || ( empty( $_GET['status'] ) && in_array( $_GET['status'], $statuses ) ) ) {
			die;
		}
	...
	public static function column_fields( $columns, $post_id ) {
		...
		switch ( $columns ) {
			...
			case 'lms_course_students':
				...
				<a href="<?php echo esc_url( admin_url( "?page=stm-lms-dashboard#/course/{$post_id}" ) ); ?>"
				   class="button action">
	...
	public static function is_instructor( $user_id = null ) {
		...
		return in_array( self::role(), $user['roles'] );
	}
