...
class projectInvoice {
    ...
    public function projectinvoiceGet($id) {
        if($id=='past') {
            ...
            $data = $this->_db->get('tms_order tpo',null,'tg.order_no projectNumber,concat(tu.vFirstName, " ", tu.vLastName) as vUserName projectManager,tg.project_name projectName, tpo.created_date projectCreated,tpo.project_status projectStatus, tpo.order_id orderId,tcl.iClientId clientId,tcl.vUserName clientName');
        } else {
    ...
    public function projectinvoiceGetOne($id, $invoiceStatus) {
        if($invoiceStatus=='past') {
            ...
            $data = $this->_db->get('tms_order tpo',null,'tg.order_no projectNumber,concat(tu.vFirstName, " ", tu.vLastName) as vUserName projectManager,tg.project_name projectName, tpo.created_date projectCreated,tpo.project_status projectStatus, tpo.order_id orderId,tco.vUserName Coordinator,tqa.vUserName QaSpecialist,tg.specialization Specialization, tg.due_date Duedate');
        } else {
            $this->_db->where('tpo.order_id',$id);
            $this->_db->where('tpo.project_status',Array('Approved','Reject','Send','Save','Cancel'),'NOT IN');
            $this->_db->join('tms_customer tc','tpo.order_id=tc.order_id','INNER');
