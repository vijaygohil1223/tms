...
class freelanceJob {
    ...
    public function freelanceJobget($id) {
    	...
    	$this->_db->where("tsv.item_status", array('In preparation','Requested','Waiting','Ongoing','Overdue','Completed','Invoice Ready','Invoice Accepted', 'Invoiced','Paid' ),"IN");
    ...
    //
    public function freelanceJobForInvoice($id) {
    	$this->_db->groupBy(' tsv.job_summmeryId ');
        $this->_db->orderBy(' DATE(tsv.due_date) ', 'DESC');
        $this->_db->where("tsv.resource = $id ");
        $this->_db->where("tsv.item_id != '0' ");
    	...
    	//$this->_db->where("tsv.item_status", array('Requested','Assigned-waiting','In-progress','Delivered','Approved','Invoice Accepted'),"IN");
    	//$this->_db->where("tsv.item_status", array('Requested','Waiting','In-progress','Ongoing','Delivered','Completed','Approved','Invoice Ready','Invoice Accepted'),"IN");
    	$this->_db->where("tsv.item_status", array('invoice ready','invoice Ready','overdue'),"IN");
        ...
        // echo $this->_db->getLastQuery();
        // exit;
        //$rowQuery = "SELECT tg.order_no, tsv.job_no, tsv.job_code, tsv.item_status, tsv.description, tsv.job_summmeryId, tsv.due_date, tsv.order_id, tsv.item_id, tsv.po_number AS poNumber, tsv.total_price, tsv.comment_read, tg.company_code, tu.freelance_currency FROM tms_summmery_view tsv INNER JOIN tms_general tg ON tsv.order_id = tg.order_id LEFT JOIN tms_users tu ON tsv.resource = tu.iUserId WHERE tsv.resource = 4 AND tsv.item_id != '0' AND tsv.item_status IN( 'invoice ready', 'invoice Ready', 'overdue' ) ORDER BY DATE(tsv.due_date)" ;
