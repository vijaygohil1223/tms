...
class dashboard {
    ...
    public function projectsOrderCount($id) {
        ...
        $qry = "SELECT COUNT(DISTINCT its.itemId) AS totalItems FROM tms_items AS its LEFT JOIN tms_general AS gen ON its.order_id = gen.order_id WHERE its.order_id != 0 AND DATE(its.due_date) = (CASE WHEN DAYOFWEEK(CURDATE() + INTERVAL 1 DAY) = 7 THEN CURDATE() + INTERVAL 3 DAY WHEN DAYOFWEEK(CURDATE() + INTERVAL 1 DAY) = 1 THEN CURDATE() + INTERVAL 2 DAY ELSE CURDATE() + INTERVAL 1 DAY END) AND its.item_status NOT IN(4, 5, 6, 8, 9) ";
    ...
    public function projectsOrderScoopGet($id) {
        ...
        if($tabName != ''){
            ...
            if($tabName == 'tab-due-tomorrow'){
                ...
                $whereCond = " AND DATE(its.due_date) = (CASE WHEN DAYOFWEEK(CURDATE() + INTERVAL 1 DAY) = 7 THEN CURDATE() + INTERVAL 3 DAY WHEN DAYOFWEEK(CURDATE() + INTERVAL 1 DAY) = 1 THEN CURDATE() + INTERVAL 2 DAY ELSE CURDATE() + INTERVAL 1 DAY END) AND its.item_status NOT IN (4, 5, 6, 8, 9) ";
            }
            if($tabName == 'tab-all'){
                //$whereCond = " ";
            }
