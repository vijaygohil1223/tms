...
class dashboard {
    ...
    public function projectsOrderCount($id) {
        ...
        $qry = "SELECT COUNT(*) AS totalItems FROM tms_items WHERE DATE(due_date) = CURDATE()";
        $data2 = $this->_db->rawQuery($qry);
        if($data2){
            $data['dueToday'] = $data2[0]['totalItems'];
        }
        $qry = "SELECT COUNT(*) AS totalItems FROM tms_items WHERE DATE(due_date) = CURDATE() + INTERVAL 1 DAY";
    ...
    public function projectsOrderScoopGet($id) {
        ...
        if($tabName != ''){
            ...
            if($tabName == 'tab-due-today'){
                $whereCond = " AND DATE(its.due_date) = CURDATE() ";
            }
            if($tabName == 'tab-due-tomorrow'){
                $whereCond = " AND DATE(its.due_date) = CURDATE() + INTERVAL 1 DAY ";
            }
            if($tabName == 'tab-all'){
                //$whereCond = " ";
            }
            ...
            if($tabName == 'tab-my-projects'){
                $whereCond = " AND cust.project_manager = $id  ";
        ...
        if($search != ''){
            $whereCond = " AND title LIKE '%$search%' ";
            $currentPage = 0;
        }
        $qry = "SELECT its.itemId from tms_items as its LEFT JOIN tms_customer AS cust ON its.order_id = cust.order_id where its.order_id != 0  $whereCond  ";
